...
function FestivalDetailsPage() {
    ...
    const fetchFestivalById = async () => {
      try {
        const response = await fetch(
          `http://localhost:3006/api/festival/${id}`
        );
  ...
  const bannerImage =
    "https://images.unsplash.com/photo-1672841821756-fc04525771c2?w=600&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NzJ8fG11c2ljJTIwZmVzdGl2YWx8ZW58MHwwfDB8fHwy";
                ...
                <Row>
                  <Col xs={6}>
                    <ListGroup variant="flush">
                      {festival.lineup
                        .slice(0, Math.ceil(festival.lineup.length / 2))
                        .map((artist, index) => (
                  ...
                  <Col xs={6}>
                    <ListGroup variant="flush">
                      {festival.lineup
                        .slice(Math.ceil(festival.lineup.length / 2))
                        .map((artist, index) => (
                          <ListGroup.Item key={index}>{artist}</ListGroup.Item>
                        ))}
                    </ListGroup>
